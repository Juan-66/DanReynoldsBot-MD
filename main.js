...
mobile: MethodMobile, 
browser: opcion == '1' ? ['DanBot-MD', 'Edge', '2.0.0'] : methodCodeQR ? ['DanReynoldsBot-MD', 'Edge', '2.0.0'] : ['Ubuntu', 'Edge', '110.0.1587.56'],
auth: {
creds: state.creds,
...
if (opts['autocleartmp'] && (global.support || {}).find) (tmp = [os.tmpdir(), 'tmp', "DanJadiBot"], tmp.forEach(filename => cp.spawn('find', [filename, '-amin', '2', '-type', 'f', '-delete'])))}, 30 * 1000)}
...
let prekey = []
let directorio = readdirSync("./DanBotSession")
let filesFolderPreKeys = directorio.filter(file => {
return file.startsWith('pre-key-')
})
prekey = [...prekey, ...filesFolderPreKeys]
filesFolderPreKeys.forEach(files => {
unlinkSync(`./DanBotSession/${files}`)
})
} 

function purgeSessionSB() {
try {
const listaDirectorios = readdirSync('./DanJadiBot/');
let SBprekey = [];
listaDirectorios.forEach(directorio => {
if (statSync(`./DanJadiBot/${directorio}`).isDirectory()) {
const DSBPreKeys = readdirSync(`./DanJadiBot/${directorio}`).filter(fileInDir => {
return fileInDir.startsWith('pre-key-')
})
SBprekey = [...SBprekey, ...DSBPreKeys];
DSBPreKeys.forEach(fileInDir => {
if (fileInDir !== 'creds.json') {
unlinkSync(`./DanJadiBot/${directorio}/${fileInDir}`)
}})
}})
...
const directories = ['./DanBotSession/', './DanJadiBot/']
